...
def extract_mffc_features(file_path: Path | str, start: int, end: int, n_mfcc: int = 25, n_fft=2048,
                          audio_base_path: Path = AUDIO_DATA_PATH) -> np.ndarray:
...
def process_audio_data(params, input_path: Path, output_path: Path, audio_base_path: Path = AUDIO_DATA_PATH):
...
if __name__ == '__main__':
    ...
    process_audio_data(params, Path('./data/balanced_data.csv'), Path('./data/mffc_data.h5'))
